...
NEW  tests/integration/run.sh                 the harness (docker-gated, TT_* overrides,
                                              ...
                                              exit 77 without docker)  — Phase 1+2 DONE
NEW  tests/integration/build-sdk.sh           SDK build of the tree's packages (dist/)
                                              — Phase 2 DONE
NEW  tests/integration/README.md              how to run locally (TT_REPO_DIR, TT_PM,
                                              TT_KEEP)                  — Phase 1+2 DONE
NEW  tests/integration/fixtures/vpn.toml      endpoint main settings (Phase 2 DONE)
NEW  tests/integration/fixtures/hosts.toml    main_hosts tt.test + cert paths (Phase 2 DONE)
NEW  tests/integration/fixtures/credentials.toml  one test user (Phase 2 DONE)
NEW  tests/integration/fixtures/whoami.py     REMOTE_ADDR HTTP oracle (Phase 2 DONE)
...
EDIT ci.yml                                   shellcheck list += tests/test_init_regenerate.sh,
                                             tests/integration/run.sh, build-sdk.sh
                                             (Phase 0/1/2 — DONE)
...
| 4 | `integration.yml`; /dev/net/tun preflight; artifact upload of logs on failure; PR run passes | **DONE (amended)** — `.github/workflows/integration.yml`: a `build-repo` matrix job (22.03.7/ipk + 25.12.5/apk, one x86-64 SDK build each via `tests/integration/build-sdk.sh`) and a per-PM `integration` matrix job that preflights /dev/net/tun (mknod/modprobe fallback), downloads the merged repositories and runs the harness with `TT_REPO_DIR` + `TT_LOGS_DIR`, uploading the failure logs. Per-ref concurrency, fail-fast off on both matrices, timeouts set. The first CI run exposed an unreproducible-locally quirk: the gh-action-sdk's package selection, driven by the generated .config, did not select the i18n companion in CI (the app and the client built; the translation — whose packaging is a side product of the app's compile — did not; every local reproduction across the SDK image, container user, git state and action flags DID build it). The workflow therefore uses the harness's own proven recipe (`build-sdk.sh`) instead of the action, and the i18n is compiled EXPLICITLY so the selection cannot skip it |
