...
| 4 | `integration.yml`; /dev/net/tun preflight; artifact upload of logs on failure; PR run passes | **DONE** — `.github/workflows/integration.yml`: a `build-repo` matrix job (22.03.7/ipk + 25.12.5/apk, one x86-64 SDK run each, both packages via `PACKAGES: luci-app-trusttunnel trusttunnel-client`) and a per-PM `integration` matrix job that preflights /dev/net/tun (mknod/modprobe fallback), downloads the merged repositories and runs the harness with `TT_REPO_DIR` + `TT_LOGS_DIR` (new harness knob for the fixed artifact location), uploading the failure logs. Per-ref concurrency, fail-fast off on both matrices, timeouts set; the fast ci.yml gates are untouched. YAML validated; both variants locally green (apk 77, opkg 78). The workflow itself runs on the next PR |
